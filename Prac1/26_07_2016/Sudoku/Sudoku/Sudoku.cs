...
class Sudoku {
    ...
    public static void DrawTable(Cell[] CellArray) { //takes in an array if Cells, top left to right, and displays the potential numbers of each cell on a table or the final answer.
        Console.SetWindowSize(90, 60); //set window size to display board
        Console.WriteLine("     0       1       2        3       4       5        6       7       8"); //display the column
        Console.WriteLine("||=======================++=======================++=======================||");
        ...
        for (int row = 0; row < 9; row++) { //row is each of the 9 rows needed to be checked
            for (int count = 0; count < 3; count++) { //count used later to distinguish between 123, 456, and 789. 
                Console.Write("|| ");
                for (int cellPos = 0; cellPos < 9; cellPos++) { //cellPos is each cell in a row that can be printed. There are 9 cells in a row
                    for (int possib = 1; possib < 4; possib++) { //possib is each possible number in the cell. Uses count to distinguish between 123, 456, and 789. 
                        if (CellArray[cellPos + (row * 9)].number == 0 && CellArray[cellPos + (row * 9)].possibilities.Contains(possib + count * 3)) { //if the Cell's number is 0 (no definite answer) and the cell contains 1, 2, or 3, display that number
                            Console.ForegroundColor = ConsoleColor.Green; //change colour to grey for possibilities. Helps distinguish a single possibility and a final answer
                            Console.Write(possib + count * 3);
                            Console.ForegroundColor = ConsoleColor.Gray;
                        }
                        else if (CellArray[cellPos + (row * 9)].number != 0) { //if the answer is definite, then display it.
                            int far = (possib + count * 3);//far is how far into the ascii cell you are.
                            if (!CellArray[cellPos + (row * 9)].original)
                                Console.Write((far == 4) ? "(" : (far == 5) ? "" + CellArray[cellPos + (row * 9)].number : (far == 6) ? ")" : " "); //if the number was not original, surround it with brackets
                            else Console.Write((far == 5) ? "" + CellArray[cellPos + (row * 9)].number : " "); //if the number is original, display it in the middle of the cell.
                        }
                        else
            ...
            if (row == 2 || row == 5 || row == 8) Console.WriteLine("||=======================++=======================++=======================||"); //last bit of formatting
            else Console.WriteLine("||-----------------------++-----------------------++-----------------------||");
        }
    }
