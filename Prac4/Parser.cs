...
public class Parser {
	...
	public const int _nonterminal = 2;
	public const int _newLine = 3;
	// terminals
	public const int EOF_SYM = 0;
	public const int terminal_Sym = 1;
	public const int nonterminal_Sym = 2;
	public const int newLine_Sym = 3;
	public const int coloncolonequal_Sym = 4;
	public const int bar_Sym = 5;
	public const int eps_Sym = 6;
	...
	static void Production() {
		...
		Expect(coloncolonequal_Sym);
		Expression();
		if (la.kind == newLine_Sym) {
			Get();
	...
	static void Term() {
		if (la.kind == terminal_Sym || la.kind == nonterminal_Sym) {
			Factor();
			while (la.kind == terminal_Sym || la.kind == nonterminal_Sym) {
				Factor();
			}
		} else if (la.kind == eps_Sym) {
			Get();
		} else SynErr(9);
	}

	static void Factor() {
		if (la.kind == terminal_Sym) {
			Get();
		} else if (la.kind == nonterminal_Sym) {
			Get();
		} else SynErr(10);
	}
...
public class Errors {
	...
	public static void SynErr (int line, int col, int n) {
		...
		switch (n) {
			...
			case 2: s = "nonterminal expected"; break;
			case 3: s = "newLine expected"; break;
			case 4: s = "\"::=\" expected"; break;
			case 5: s = "\"|\" expected"; break;
			case 6: s = "\"eps\" expected"; break;
			case 7: s = "??? expected"; break;
			case 8: s = "invalid Production"; break;
			case 9: s = "invalid Term"; break;
			case 10: s = "invalid Factor"; break;
